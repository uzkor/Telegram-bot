...
def parse_weekly_txt(text: str):
    ...
    for block in blocks:
        ...
        m = re.match(r"^JAVOB:\s*([A-D])$", ans_line, re.IGNORECASE)
        if m and len(options) == 4:
            answer_idx = "ABCD".index(m.group(1).upper())
            questions.append({"q": q_text, "options": options, "answer": answer_idx})
    return questions


async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin .txt fayl yuborganda haftalik savollarni yuklab oladi."""
    user = update.effective_user
    if user.id != ADMIN_ID:
        return
    ...
    questions = parse_weekly_txt(text)
    if not questions:
        await update.message.reply_text(
            ...
            "D) Variant 4\n"
            "JAVOB: A\n\n"
            "(har bir savol orasida bo'sh qator bo'lishi kerak)"
        )
        return

    context.bot_data["weekly_questions"] = questions

    await update.message.reply_text(
        f"✅ {len(questions)} ta savol qabul qilindi!\n\n"
        f"Bu hafta shanba kuni soat 20:00da, hozirgacha botda ro'yxatdan o'tgan "
        f"barcha foydalanuvchilarga avtomatik yuboriladi.\n\n"
        f"Hoziroq sinab ko'rish uchun /test_yubor buyrug'ini ishlatishingiz mumkin."
    )


async def manual_send_weekly(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin uchun: haftalik testni qo'lda darhol yuborish (sinov uchun)."""
    user = update.effective_user
    if user.id != ADMIN_ID:
        return
    questions = context.bot_data.get("weekly_questions")
    if not questions:
        await update.message.reply_text("Hozircha haftalik savollar yuklanmagan. Avval .txt fayl yuboring.")
        return
    await send_weekly_test_to_all(context, questions)
    await update.message.reply_text("✅ Haftalik test barcha obunachilarga yuborildi.")
...
async def manual_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != ADMIN_ID:
        return
    await announce_leaderboard(context)
    await update.message.reply_text("✅ Reyting e'lon qilindi.")


async def send_weekly_test_to_all(context: ContextTypes.DEFAULT_TYPE, questions):
    subscribers = get_all_subscribers()
    test_date = datetime.now().date().isoformat()
    context.bot_data["weekly_test_date"] = test_date
    context.bot_data["weekly_active_users"] = {}

    for telegram_id, username, full_name in subscribers:
        try:
            context.bot_data["weekly_active_users"][telegram_id] = {
                "username": username,
                "full_name": full_name,
                "current_q": 0,
                "score": 0,
            ...
            await context.bot.send_message(
                chat_id=telegram_id,
                text=f"📝 Haftalik test boshlandi! Jami {len(questions)} ta savol.\n"
                     f"Tezroq va to'g'ri javob bering - bu reytingdagi o'rningizga ta'sir qiladi!"
            )
            await send_weekly_question(context, telegram_id, questions)
        except (Forbidden, BadRequest):
            continue
        except Exception as e:
            logging.warning(f"Foydalanuvchi {telegram_id} ga yuborilmadi: {e}")


async def send_weekly_question(context: ContextTypes.DEFAULT_TYPE, telegram_id, questions):
    state = context.bot_data["weekly_active_users"].get(telegram_id)
    if not state:
        return
    idx = state["current_q"]
    if idx >= len(questions):
        await finish_weekly_test(context, telegram_id, questions)
        return
    q = questions[idx]
    kb = ReplyKeyboardMarkup([[opt] for opt in q["options"]], resize_keyboard=True, one_time_keyboard=True)
    await context.bot.send_message(
        chat_id=telegram_id,
        text=f"{idx + 1}/{len(questions)}) {q['q']}",
        reply_markup=kb
    )


async def finish_weekly_test(context: ContextTypes.DEFAULT_TYPE, telegram_id, questions):
    state = context.bot_data["weekly_active_users"].pop(telegram_id, None)
    if not state:
        return
    ...
    await context.bot.send_message(
        chat_id=telegram_id,
        text=f"✅ Haftalik test tugadi!\nNatijangiz: {state['score']}/{len(questions)}\n"
             f"Vaqt: {int(duration)} soniya\n\nReyting tez orada e'lon qilinadi."
    )


async def announce_leaderboard(context: ContextTypes.DEFAULT_TYPE):
    """Reytingni adminga va umumiy kanalga yuboradi, har bir ishtirokchining
    @username va nechanchi o'rinda turganini ko'rsatadi."""
    test_date = context.bot_data.get("weekly_test_date") or datetime.now().date().isoformat()
    results = get_weekly_leaderboard(test_date)
    if not results:
        text = "Bugun hech kim haftalik testda ishtirok etmadi."
    else:
        lines = ["🏆 HAFTALIK TEST NATIJALARI 🏆\n"]
        for i, (username, full_name, score, total, duration) in enumerate(results, start=1):
            uname = f"@{username}" if username else full_name
            lines.append(f"{i}-o'rin: {uname} — {score}/{total} ball — {int(duration)} soniya")
        text = "\n".join(lines)
    ...
    # Umumiy kanalga yuborish (kanal @username yoki -100... ID bo'lishi kerak)
    ...
    if channel and (channel.startswith("@") or channel.startswith("-100")):
        try:
            ...
            logging.warning(f"Reyting kanalga yuborilmadi: {e}")


# ============================================================
# JOB QUEUE: HAR SHANBA 20:00DA AVTOMATIK YUBORISH
# ============================================================

async def weekly_job(context: ContextTypes.DEFAULT_TYPE):
    questions = context.bot_data.get("weekly_questions")
    if not questions:
        logging.info("Haftalik savollar yuklanmagan, test yuborilmadi.")
        return
    await send_weekly_test_to_all(context, questions)
...
# ============================================================
# HAFTALIK TEST JAVOBLARINI ASOSIY HANDLERLARDAN OLDIN USHLAB QOLISH
# ============================================================

async def weekly_intercept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """group=-1 da ishlaydi. Agar foydalanuvchi haftalik testda bo'lsa,
    javobni qabul qiladi va boshqa handlerlarga o'tishni to'xtatadi."""
    if update.message is None or not update.message.text:
        return

    telegram_id = update.effective_user.id
    active = context.bot_data.get("weekly_active_users", {})
    state = active.get(telegram_id)
    if not state:
        return  # haftalik testda emas -> ConversationHandler ishlashda davom etadi

    questions = context.bot_data.get("weekly_questions") or []
    idx = state["current_q"]
    if idx >= len(questions):
        return
...
def main():
    ...
    # group=-1: ConversationHandlerdan OLDIN ishlaydi.
    # Agar foydalanuvchi haftalik testda bo'lsa javobni ushlab qoladi,
    # aks holda hech narsa qilmaydi va keyingi handlerlarga o'tadi.
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, weekly_intercept), group=-1)
    ...
    # Admin: .txt fayl yuklash orqali haftalik savollarni belgilash
    app.add_handler(MessageHandler(filters.Document.ALL, handle_document))

    # Admin buyruqlari (sinov uchun)
    app.add_handler(CommandHandler("test_yubor", manual_send_weekly))
    app.add_handler(CommandHandler("reyting", manual_leaderboard))

    # ---- JOB QUEUE: har shanba 20:00da avtomatik yuborish ----
    job_queue = app.job_queue
    # Shanba = 5 (0=Dushanba ... 5=Shanba, 6=Yakshanba)
    job_queue.run_daily(weekly_job, time=dtime(hour=20, minute=0), days=(5,))
    # Reytingni testdan keyin (masalan 20:40da) e'lon qilish
    job_queue.run_daily(leaderboard_job, time=dtime(hour=20, minute=40), days=(5,))
