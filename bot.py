...
MINI_APP_URL = "https://uzkor.github.io/korean-miniapp/"  # <-- bu yerga haqiqiy URL qo'yiladi

# Kunlik test poll'lari yuboriladigan guruh ID
GROUP_CHAT_ID = -1001297463776

DEFAULT_POLL_SECONDS_GROUP = 35   # kunlik guruh testi uchun
DEFAULT_POLL_SECONDS_PERSONAL = 30  # shaxsiy /start testi uchun
...
async def begin_adaptive_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Test boshlanadi. Jami {PERSONAL_TEST_QUESTION_COUNT} ta savol, har biriga {DEFAULT_POLL_SECONDS_PERSONAL} soniya vaqt beriladi.\n"
...
async def send_adaptive_question(chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = await context.bot.send_poll(
        ...
        is_anonymous=False,
        open_period=DEFAULT_POLL_SECONDS_PERSONAL,
    )
    ...
    context.user_data["current_poll_deadline"] = time.time() + DEFAULT_POLL_SECONDS_PERSONAL + 1
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Jami: {total} ta\n\n"
        f"Bu savollar:\n"
        f"- Har bir foydalanuvchining /start testida (adaptiv, 10ta savol, 30 sek)\n"
        f"- Har kuni 20:00da guruhga yuboriladigan kunlik testda (random 10ta, 35 sek)\n"
        f"ishlatiladi.\n\n"
...
async def send_daily_test_to_group(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Barcha savollarni bitta ro'yxatga yig'ib, random 10ta tanlaymiz
    all_questions = []
    ...
    try:
        await context.bot.send_message(
            ...
            text=f"📝 Bugungi test boshlandi! Jami {count} ta savol.\n"
                 f"Har bir savolga {DEFAULT_POLL_SECONDS_GROUP} soniya vaqt beriladi. Omad! 🍀"
        )
    except Exception as e:
...
# ============================================================
# MINI APP (Web App) UCHUN STATIK FAYL SERVERI
# ============================================================
...
def run_webapp_server():
    """index.html va boshqa statik fayllarni serve qiladi (Mini App uchun).
    Replit'da bu port avtomatik tashqi URL'ga ulanadi."""
    handler = lambda *args, **kwargs: SimpleHTTPRequestHandler(*args, directory=WEBAPP_DIR, **kwargs)
    server = ThreadingHTTPServer(("0.0.0.0", WEBAPP_PORT), handler)
    logging.info(f"Mini App server ishga tushdi: port {WEBAPP_PORT}, papka: {WEBAPP_DIR}")
...
def main():
    ...
    # Mini App uchun statik fayl serverini alohida thread'da ishga tushiramiz
